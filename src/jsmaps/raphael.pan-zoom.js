...
(function () {
    ...
    var panZoomFunctions = {
        ...
        PanZoom = function (el, options) {
            ...
            settings.originWidth = options.originSize.width || paper.width;
            settings.originHeight = options.originSize.height || paper.height;
            ...
            function repaint() {
                ...
                var newWidth = settings.originWidth * (1 - (me.currZoom * settings.zoomStep)),
                    newHeight = settings.originHeight * (1 - (me.currZoom * settings.zoomStep));
    
                if (me.currPos.x < 0) {
                    me.currPos.x = 0;
                } else if (me.currPos.x > (settings.originWidth * me.currZoom * settings.zoomStep)) {
                    me.currPos.x = (settings.originWidth * me.currZoom * settings.zoomStep);
                }
            ...
            container.onmousedown = function (e) {
                ...
                document.onmouseup = container.onmouseup;
                if (evt.preventDefault) {
                    evt.preventDefault();
                } else {
                    evt.returnValue = false;
